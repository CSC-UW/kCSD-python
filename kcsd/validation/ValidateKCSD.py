...
class ValidateKCSD(object):
    ...
    def integrate(self, csd_at, true_csd, ele_loc, h, csd_lims=None):
        ...
        if self.dim == 1:
            ...
            for i in range(Ny):
                integral_1D[i] = simpson(y[:, i], ylin)  # I changed the integral
            Integral = simpson(integral_1D, xlin)  # then an integral over the result
        else:
            ...
            for j in range(Ny):
                Iz = np.zeros(Nz)
                for i in range(Nz):
                    Iz[i] = simpson(z[:, j, i], csd_lims[2])
                Iy[j] = simpson(Iz, csd_lims[1])
            Integral = simpson(Iy, csd_lims[0])
        return Integral
