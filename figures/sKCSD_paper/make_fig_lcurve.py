...
def make_plot_fig2(sim_results):
    ...
    plt.fill_between(
        noise_lvl, mn_rms - st_rms, mn_rms + st_rms, alpha=0.3, color="blue"
    )
    ...
    plt.fill_between(
        noise_lvl, mn_rms - st_rms, mn_rms + st_rms, alpha=0.3, color="green"
    )
    ...
    plt.fill_between(
        noise_lvl, mn_lam - st_lam, mn_lam + st_lam, alpha=0.3, color="blue"
    )
    ...
    plt.fill_between(
        noise_lvl, mn_lam - st_lam, mn_lam + st_lam, alpha=0.3, color="green"
    )
...
def make_plots(
    ...
    ax1 = plt.subplot(131)
    ax1.plot(
        ele_y, pots * 1e3, "r", marker="o", linewidth=0, label="measured potential"
    )
    ...
    true_csd_error = np.linalg.norm(
        true_csd / np.max(abs(true_csd)) - est_csd / np.max(abs(est_csd))
    )
...
def generate_csd_1D(
    ...
    src_peak = np.exp(-((csd_x - 0.25) ** 2) / (2.0 * src_width))
    src_thr1 = 0.5 * np.exp(-((csd_x - 0.5) ** 2) / (2.0 * src_width))
    src_thr2 = 0.5 * np.exp(-((csd_x - 0.6) ** 2) / (2.0 * src_width))
...
def main_loop(src_width, total_ele, inpos, lpos, nm, noise=0, srcs=1):
    ...
    for i, noise in enumerate(n_spec):
        ...
        k, est_csd, est_pot = do_kcsd(
            ele_y, pots, h=1.0, gdx=gdX, xmin=x_lims[0], xmax=x_lims[1], n_src_init=1e4
        )
...
if __name__ == "__main__":
    ...
    if figs1_and_fig2:
        for iname, name in enumerate(names):
            for src in range(10):
                ...
                if not os.path.isdir(mypath):
                    os.makedirs(mypath)
                os.chdir(mypath)
                RMS_wek, LandR = main_loop(
                    src_width, total_ele, inpos, lpos, name, noise=noise_lvl, srcs=src
                )
        ...
        name = "lc"
        main_loop(src_width, total_ele, inpos, lpos, name, noise=noise_lvl[:1], srcs=0)
