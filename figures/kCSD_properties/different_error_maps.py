...
def integrate_2d(csd_at, true_csd, ele_pos, h, csd_lims):
    ...
    for i in range(Ny):
        integral_1D[i] = simpson(y[:, i], ylin)
    integral = simpson(integral_1D, xlin)
    return integral
...
def point_errors2(true_csd, est_csd):
    ...
    err2 = abs(true_csd.reshape(true_csd.size, 1) - est_csd.reshape(est_csd.size, 1))
    err2 /= (
        abs(true_csd.reshape(true_csd.size, 1)) + epsilon
    )  # *np.max(abs(true_csd.reshape(true_csd.size, 1)))
    err = err2.reshape(true_csd.shape)
    return err


def sigmoid_mean(error):
    sig_error = 2.0 / (1.0 + np.exp(-error)) - 1.0
    return sig_error
...
def calculate_rdm(true_csd, est_csd):
    rdm = abs(
        ...
        / (np.linalg.norm(est_csd.reshape(est_csd.size, 1)))
        - true_csd.reshape(true_csd.size, 1)
        / (np.linalg.norm(true_csd.reshape(true_csd.size, 1)))
    )
    rdm *= np.linalg.norm(true_csd.reshape(true_csd.size, 1)) / np.max(
        abs(true_csd.reshape(true_csd.size, 1))
    )
...
def calculate_mag(true_csd, est_csd):
    epsilon = np.max(abs(true_csd.reshape(true_csd.size, 1)))
    mag = abs(est_csd.reshape(est_csd.size, 1)) / (
        abs(true_csd.reshape(true_csd.size, 1)) + epsilon
    )
...
def do_kcsd(CSD_PROFILE, data, csd_seed, prefix, missing_ele):
    ...
    im = ax.contourf(k.estm_x, k.estm_y, error1, levels=levels_kcsd, cmap=cm.Greys)
    ax.set_xlim([0, 1])
    ax.set_ylim([0, 1])
    ax.set_xticks([0, 0.5, 1])
    ax.set_yticks([0, 0.5, 1])
    ...
    levels_kcsd = np.linspace(0, t_max, 16, endpoint=True)
    im = ax.contourf(k.estm_x, k.estm_y, error2, levels=levels_kcsd, cmap=cm.Greys)
    ax.set_xlim([0, 1])
    ax.set_ylim([0, 1])
    ax.set_xticks([0, 0.5, 1])
    ax.set_yticks([0, 0.5, 1])
    ...
    levels_kcsd = np.linspace(0, t_max, 16, endpoint=True)
    im = ax.contourf(k.estm_x, k.estm_y, error3, levels=levels_kcsd, cmap=cm.Greys)
    ax.set_xlim([0, 1])
    ax.set_ylim([0, 1])
    ax.set_xticks([0, 0.5, 1])
    ax.set_yticks([0, 0.5, 1])
    ...
    levels_kcsd = np.linspace(0, t_max, 16, endpoint=True)
    im = ax.contourf(k.estm_x, k.estm_y, error4, levels=levels_kcsd, cmap=cm.Greys)
    ax.set_xlim([0, 1])
    ax.set_ylim([0, 1])
    ax.set_xticks([0, 0.5, 1])
    ax.set_yticks([0, 0.5, 1])
...
if __name__ == "__main__":
    ...
    prefix = (
        "/home/mkowalska/Marta/kCSD-python/figures/kCSD_properties/small_srcs_all_ele"
    )
